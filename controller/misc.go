...
func SendEmailVerification(c *gin.Context) {
	...
	if config.EmailDomainRestrictionEnabled {
		...
		if !allowed {
			c.JSON(http.StatusOK, gin.H{
				"success": false,
				"message": "The administrator has enabled email domain whitelist, and your email domain is not in the whitelist",
			})
			return
		}
	}
...
func SendPasswordResetEmail(c *gin.Context) {
	...
	content := message.EmailTemplate(
		...
		fmt.Sprintf(`
			...
			<p style="color: #666;">If the button doesn't work, please copy and paste the following link in your browser:</p>
			<p style="background-color: #f8f8f8; padding: 10px; border-radius: 4px; word-break: break-all;">%s</p>
			<p style="color: #666;">The reset link is valid for %d minutes. If you did not request this, please ignore.</p>
