...
func (user *User) Insert(ctx context.Context, inviterId int) error {
	...
	if inviterId != 0 {
		if config.QuotaForInvitee > 0 {
			...
			RecordLog(ctx, user.Id, LogTypeSystem, fmt.Sprintf("Bonus for using invitation code %s", common.LogQuota(config.QuotaForInvitee)))
		}
		if config.QuotaForInviter > 0 {
			_ = IncreaseUserQuota(inviterId, config.QuotaForInviter)
			RecordLog(ctx, inviterId, LogTypeSystem, fmt.Sprintf("Bonus for inviting user %s", common.LogQuota(config.QuotaForInviter)))
		}
	}
